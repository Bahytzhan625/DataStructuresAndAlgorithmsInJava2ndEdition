...
class BTree {
	...
	/**
	 * Получить соответствующего потомка
	 *
	 ...
	 * @param order порядок дерева использующегося для сортировки
	 ...
	 *
	 * @return новый узел соответствующий порядку дерева
	 */
	 ...
	 *
	 * @param brokenBNode разбиваемый узел(преполагается, что разбиваемый узел полон)
	 * @param newItem 	  вставляемый элемент данных
	 */
		...
		/* разбиваемый узел не является корнем */
			...
			if (parentNode.isFull()) {
				...
				/* разбиваемый узел имеет потомков */
					...
					/* присоединяем новых потомков к разбиваемому узлу */
			...
			/* родительский узел не полон, можно вставлять элемент в него */
				...
				if (brokenBNode.isLeaf()) {
					...
					/* разбиение происходит в правом узле элемента */
				...
				/* разбиваемый узел имеет потомков */
					...
					/* распределяем дочерние элементы родительского узла */
					if (parentNode.getItem(LEFT_ITEM).getKey() > childNodes.getValue().getItem(LEFT_ITEM).getKey()) {
						/* смещаем среднего потомка родительского элемента на одну позицию правее */
						parentNode.connectChild(RIGHT_CHILD, parentNode.disconnectChild(CENTRAL_CHILD));
	...
	/**
	 * Метод создающий новые дочерние узлы для их дальнейшего приккрепления к родительскому узлу
	 *
	...
	private Map.Entry<BNode, BNode> getNewChildNodes(BNode highOrderNode, BNode brokenBNode, Item newItem) {
		...
		if (highOrderNode.getItem(LEFT_ITEM).getKey() == newItem.getKey()) {
			...
			if (brokenBNode.getChild(LEFT_CHILD).getItem(LEFT_ITEM) == null) {
				/* присоединяем нового правого потомка к новому левому потомку */
		...
		/* источник разбиение средний потомок */
